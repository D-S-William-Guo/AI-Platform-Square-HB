# GOVERNANCE（当前定版）

## 1. 产品边界

- 对外聚焦：双榜单、展示型应用视图、申报入口
- 首页默认展示排行榜；集团应用和省内应用统一进入应用视图，通过应用来源筛选区分
- 集团应用和省内应用均定位为信息展示，不作为平台内跳转使用入口
- 访问权限分层：
  - 匿名可读：首页列表、应用详情、榜单只读
- 登录后可访问：我要申报、我的申报
  - 管理能力：仅管理员登录后访问
- 已通过省内应用的申报人修改当前应用信息时，必须提交“应用变更申请”，由管理员审核通过后才更新正式应用。
- 被拒绝申报可由原申报人修改后重新提交审核；待审核申报仍只允许修改或撤回。
- 交互审计基线：
  - 前端关键意图与路由守卫事件写入后端 `ActionLog`
  - 审计事件接口使用白名单与限频保护
- 管理能力仅管理员可见，且后端必须有权限保护
- 远程部署以发布包为准，远程主机不承担前端构建
- 前端发布物支持按目标入口选择根路径或子路径构建
- 子路径发布时，页面、前端 API 与媒体资源统一跟随同一前缀，避免额外占用宿主站点根 `/api`

## 2. 榜单规则

- 双榜单固定为：
  - `excellent`：总应用榜
  - `trend`：增长趋势榜
- 当前榜单仅展示省内应用，集团应用不参与榜单计算和对外榜单展示
- 默认核心维度：
  - 总应用榜：用户满意度、业务价值、使用活跃度、稳定性和安全性
  - 增长趋势榜：使用活跃度、增长趋势、用户增长
- 系统默认权重统一为 `1.0`
- “技术创新性”“市场热度”保留为系统维度，但不是默认榜单主维度

## 3. 真相源

- 榜单参与与控制输入唯一真相源：`AppRankingSetting`
- 对外榜单读取唯一真相源：`HistoricalRanking`
- `App` / `Submission` 中的 `ranking_*` 字段仅保留兼容或展示用途
- `App` 是当前正式展示态；`Submission` 保留申报来源记录；`AppChangeRequest` 记录已上架应用的修改申请与审核留痕。
- 应用改名或展示信息变更后，榜单参与、因子评分和参评设置继续通过同一 `app_id` 关联，不因名称变化重建。
- 已发布的 `HistoricalRanking.app_name` / `app_org` 是发布当时快照，不随应用后续改名回写；后续新发布快照使用当时最新 `App` 信息。

## 4. 数据库与初始化

- 结构变更统一走 `alembic upgrade head`
- 新增应用变更申请等结构时，仍只通过 Alembic 迁移落库，不用清库重建。
- `init-base` 只负责基础初始化，不覆盖已有默认密码
- `reset-default-users` 用于显式重置 `zhangsan` / `lisi`
- `sync-system-presets` 用于显式同步系统维度与系统榜单默认值（含维度关联）
- `ranking_config_dimensions` 是榜单配置-维度关联表（2026-06-01 由 dimensions_config JSON TEXT 迁移），有 FK 约束到 ranking_configs 和 ranking_dimensions
- `ranking_type` 列已从 rankings / historical_rankings / ranking_audit_logs 三表删除（原与 ranking_config_id 双写冗余，现已统一为 ranking_config_id）

## 5. 身份模式

- 默认身份模式：`local`
- 会话策略：仅本地会话 cookie（不启用 remember me）
- 本地密码策略：至少 10 位，且大写字母、小写字母、数字、符号四类中至少三类
- 初始密码和管理员重置密码都按临时密码处理，用户下次登录后必须先改为个人强口令
- 用户自助改密成功后保留当前会话，并撤销该用户其他未过期会话
- 不做外部系统用户同步；外部系统挂接默认按游客/只读普通用户进入首页
- 预留：
  - `oa`
  - `external_sso`
- 当前轮次只做统一身份适配层和登录入口预留，不直接接真实 OA / 第三方协议

## 6. 文档治理规则

- 根 `README.md` 只承担项目首页、最短入口和文档导航职责，不长期维护完整运维细节。
- `docs/README.md` 是当前有效文档索引，新增、重命名或废弃有效文档时必须同步更新。
- `AGENTS.md` 用于仓库级智能体工作约束、文档入口说明和最近关键文档变更摘要。
- `backend/scripts/README.md` 只解释脚本用途，不承担后端主入口或治理结论职责。
- `docs/archive/` 仅保留历史材料，不作为当前产品、部署或运维规则入口。

## 7. 规则变化时的同步要求

- 产品边界、榜单规则、数据真相源变化时，同步检查：
  - `docs/GOVERNANCE.md`
  - 根 `README.md`
  - `docs/README.md`
- 开发、部署、数据库命令变化时，同步检查：
  - 根 `README.md`
  - `docs/dev-setup.md`
  - `docs/dev-workflow.md`
  - `docs/db-migration-sop.md`
  - `docs/README.md`
- 文档入口或文档职责变化时，同步检查：
  - `AGENTS.md`
  - 根 `README.md`
  - `docs/README.md`

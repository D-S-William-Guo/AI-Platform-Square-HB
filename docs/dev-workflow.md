...
- 未登录不可访问：
  - 我的申报
  - 管理员功能页
- 未登录点击“我要申报”会跳转登录，登录成功后自动返回并打开申报弹窗
- 未登录态展示独立“管理员登录”入口；非管理员登录后不可进入管理页
- 已拒绝申报可在“我的申报”修改后重提；已通过申报可提交应用变更申请，管理员在“申报审核”处理。
- 应用改名后，当前应用和新发布榜单使用新名称，历史榜单快照仍展示发布当时名称。
- 审计链路可观测：关键事件可在 `/api/action-logs` 查询，并可按 `request_id` 关联

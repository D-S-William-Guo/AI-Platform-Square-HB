...
- 首页对外聚焦：双榜单、展示型应用视图、申报入口
- 首页默认展示排行榜；集团应用和省内应用统一进入应用视图，通过应用来源筛选区分
- 集团应用和省内应用均定位为信息展示，不作为平台内跳转使用入口
- 访问分层：
  - 匿名可读：首页列表、应用详情、榜单
  - 登录后可用：我要申报、我的申报
  - 管理员登录后可用：管理能力入口
...
- 双榜单固定为：
  - `excellent`：总应用榜
  - `trend`：增长趋势榜
- 榜单当前只展示省内应用，集团应用不参与当前榜单
- 默认身份模式：`local`
- 本地密码必须为强口令：至少 10 位，且大写字母、小写字母、数字、符号四类中至少三类；初始或管理员重置后的密码必须由用户首次登录后修改
